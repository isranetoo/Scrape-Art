...
class SessaoJurisprudencia:
    ...
    def obter_documentos(self):
        ...
        try:
            if not self.token_desafio or not self.resposta_captcha:
                print("Token Desafion ou resposta ausentes")
                return
            token_captcha = self.token_desafio
            url_documentos = f"{URL_DOCUMENTOS}?tokenCaptcha={self.token_captcha}"
    ...
    def enviar_documentos(self):
        """Realizando o POST para enviar o documento com o token a resposta do Captcha"""
        try:
            if not self.token_desafio or not self.resposta_captcha:
                print("Token de desafio ou resposta do captcha ausentes")
                return
            ...
            print(f"Erro ao realizar o envio do POST: {e}") 
