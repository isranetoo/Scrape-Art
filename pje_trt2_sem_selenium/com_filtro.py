...
class SessaoJurisprudencia:
    ...
    def salvar_documentos(self, documentos):
        def filtrar_documento(doc):
            try:
                return (
                    (doc.get("classeJudicial", "") == "") and
                    (doc.get("classeJudicialSigla", "") == "") and
                    (doc.get("dataDistribuicao", "") == "") and
                    (doc.get("dataPublicacao", "") == "") and
                    (doc.get("instancia", "") == "") and
                    (doc.get("magistrado", [""]) == [""]) and
                    (doc.get("meioTramitacao", "") == "") and
                    ("" in doc.get("", [])) and
                    (doc.get("orgaoJulgador", "") == "") and
                    (doc.get("orgaoJulgadorColegiado", "") == "") and
                    (doc.get("processo", "") == "") and
                    (doc.get("sigiloso",) == "") and
        ...
        documentos_filtrados = [doc for doc in documentos.get('resultados', []) if filtrar_documento(doc)]

        if not documentos_filtrados:
            print("Nenhum documento corresponde aos critérios de filtro.")
            return

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        arquivo_nome = f"documentos_filtrados_{timestamp}.json"
        pasta = "documentos"
